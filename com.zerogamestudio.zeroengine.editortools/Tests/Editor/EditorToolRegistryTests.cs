...
namespace ZeroEngine.EditorTools.Tests
{
    ...
    public sealed class EditorToolRegistryTests
    {
        ...
        [Test]
        public void ZeroEnginePresets_ExposeSourceFastSlowAndAuditLayers()
        {
            var tasks = ZeroEngineTestRunnerPresets.ToolWindowTasks();
            ...
            Assert.That(ZeroEngineTestRunnerPresets.QualityGateEditModeLayers().Select(task => task.Id), Is.EqualTo(new[]
            {
                "zeroengine.source-editmode",
                "zeroengine.fast-editmode",
                "zeroengine.slow-editmode"
        ...
        [Test]
        public void ZeroEngineSourcePreset_UsesGroupFilterInsteadOfRunAll()
        {
            Filter filter = ZeroEngineTestRunnerPresets.SourceEditMode().CreateFilter();

            Assert.That(filter.testMode, Is.EqualTo(TestMode.EditMode));
            Assert.That(filter.groupNames, Is.Not.Empty);
            Assert.That(filter.assemblyNames, Is.Empty);
            Assert.That(filter.categoryNames, Is.Empty);
        }

        [Test]
        public void ZeroEngineFastAndSlowPresets_AreDisjointAssemblies()
        {
            CollectionAssert.IsEmpty(
                ZeroEngineTestRunnerPresets.FastEditModeAssemblies
                    .Intersect(ZeroEngineTestRunnerPresets.SlowEditModeAssemblies));
            Assert.That(
                ZeroEngineTestRunnerPresets.FullEditModeAssemblies.Length,
                Is.EqualTo(
                    ZeroEngineTestRunnerPresets.FastEditModeAssemblies.Length
                    + ZeroEngineTestRunnerPresets.SlowEditModeAssemblies.Length));
        }
