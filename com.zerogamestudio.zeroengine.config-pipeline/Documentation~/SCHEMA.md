...
`x-zgs-sheet` maps an array of objects to a worksheet. Each table has exactly one
string `x-zgs-primary-key`. A child array additionally declares a synthetic
`x-zgs-parent-key` column and an integer `x-zgs-order-field`; the parent key does
not enter runtime JSON. Numeric representation is fixed with `x-zgs-number-type`.
