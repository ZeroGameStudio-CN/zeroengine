...
- 首页/系统/帮助三页采用自适应布局：首页内容显示常用工作流，左侧只承载可排序、可折叠的项目面板树。
- 首页搜索直接覆盖已安装模块的动作和资料；同一窗口的次要动作收入“更多”，已有内嵌面板的导航入口自动隐藏，文档类 entry 通过 `contentType: "reference"` 独立进入相关资料。
- 说明、使用方法、安全影响和技术来源进入 tooltip 或独立帮助页，不占用工作面板正文。
- 本机记忆上次页面、面板、搜索和主要滚动位置；旧四页导航和工具库筛选状态会迁移到首页并清理。
