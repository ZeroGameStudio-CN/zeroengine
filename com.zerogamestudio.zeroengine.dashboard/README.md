...
- 侧栏面板在空间允许时显示 `ZE`、`ZE·POB`、`POB` 等紧凑归属标识，内容区顶部分别显示 `ZE 通用`、`ZE 能力 · POB 适配`、`POB 项目`；窄侧栏优先保留面板名称。
...
- 工作区侧栏和内容区新增响应式模块归属标识，明确区分 ZE 通用模块、基于真实依赖关系的 `ZE·项目` 薄适配与纯项目模块；窄侧栏不会因徽标截断原本可完整显示的面板名称。
