...
- 新增 Editor-only `com.zerogamestudio.zeroengine.project-atlas` `1.1.0`，直接依赖 editor-ui `1.5.0` 与 Newtonsoft.Json `3.2.1`；实现严格技术目录、覆盖与 Markdown 投影，以及独立的人类功能目录与 typed route 合同。
...
- ZE Project Atlas：最终 `34/34` 通过，其中功能目录 `12/12`；Dashboard route registry `41/41`、editor-ui route contract `12/12` 通过；Dashboard descriptors `9`、Editor UI 静态合同 `descriptors=31 / coverage=33 / modules=31` 通过。
