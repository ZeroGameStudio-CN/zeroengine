...
namespace ZGS.DataToolkit.Editor.Tests
{
    public sealed class DataAuthoringDetailPipelineTests
    {
        ...
        {
            var path = fullName switch
            {
                "ZGS.DataToolkit.Editor.DataAuthoringWindow" => PackageFile("Editor", "Authoring", "DataAuthoringWindow.cs"),
                "ZGS.DataToolkit.Editor.DataAuthoringInspectorHost" => PackageFile("Editor", "Authoring", "DataAuthoringInspectorHost.cs"),
                _ => null
            };

            Assert.That(path, Is.Not.Null, $"Could not map script source for {fullName}.");
            Assert.That(File.Exists(path), Is.True, $"Could not find script source for {fullName} at {path}.");
            return File.ReadAllText(path);
        }

        private static string PackageFile(params string[] parts)
        {
            return Path.Combine(new[] { "Packages", "com.zerogamestudio.zeroengine.data-toolkit" }.Concat(parts).ToArray());
        }
