...
namespace ZeroEngine.World.Tests.Editor
{
    public sealed class WorldGraphValidationKernelTests
    {
        ...
        [Test]
        public async Task WorldStreamingService_LoaderException_ReturnsLoaderFailed()
        {
            var graph = CreateSingleCellGraph();
            try
            {
                var loader = new TestWorldCellLoader
                {
                    LoadAsync = (_, _, _) => throw new InvalidOperationException("load exploded")
                };
                ...
                Assert.AreEqual(WorldStreamingResultStatus.LoaderFailed, result.Status);
                StringAssert.Contains("load exploded", result.Message);
                Assert.IsFalse(streaming.IsCellLoaded("cell.street"));
            }
            finally
            {
                UnityEngine.Object.DestroyImmediate(graph);
        ...
        [Test]
        public async Task WorldStreamingService_ReadinessFailure_UnloadsLoadedCell()
        {
            var graph = CreateSingleCellGraph();
            try
            {
                var loader = new TestWorldCellLoader();
                var readiness = new TestWorldCellReadinessService
                {
                    PrepareAsync = (cell, _, _) => Task.FromResult(
                        WorldCellReadinessResult.Failed(cell.CellId, "navigation missing"))
                };
                ...
                Assert.AreEqual(WorldStreamingResultStatus.ReadinessFailed, result.Status);
                StringAssert.Contains("navigation missing", result.Message);
                Assert.AreEqual(1, loader.UnloadCount);
                Assert.IsFalse(streaming.IsCellLoaded("cell.street"));
            }
            finally
            {
        ...
        [Test]
        public void WorldTestsAsmdef_ReferencesWorldEditorAssembly()
        {
            string asmdef = File.ReadAllText(
                "Packages/com.zerogamestudio.zeroengine.world/Tests/Editor/ZeroEngine.World.Tests.Editor.asmdef");

            StringAssert.Contains(@"""ZeroEngine.World.Editor""", asmdef);
        }

        private static WorldGraphSO CreateSingleCellGraph()
        {
            var graph = ScriptableObject.CreateInstance<WorldGraphSO>();
            graph.ConfigureForTests(
                "world.longleji",
        ...
        private sealed class TestWorldCellLoader : IWorldCellLoader
        {
            public Func<WorldCellDefinition, WorldCellLayer, CancellationToken, Task<WorldCellOperationResult>> LoadAsync { get; set; }
            public Func<WorldCellDefinition, CancellationToken, Task<WorldCellOperationResult>> UnloadAsync { get; set; }
                ...
                CancellationToken cancellationToken)
            {
                if (LoadAsync != null)
                {
                    return LoadAsync(cell, layers, cancellationToken);
                }
                ...
                CancellationToken cancellationToken)
            {
                ...
                if (UnloadAsync != null)
                {
                    return UnloadAsync(cell, cancellationToken);
                }
        ...
        private sealed class TestWorldCellReadinessService : IWorldCellReadinessService
        {
            public Func<WorldCellDefinition, WorldCellLayer, CancellationToken, Task<WorldCellReadinessResult>> PrepareAsync { get; set; }
                ...
                CancellationToken cancellationToken)
            {
                ...
                {
                    return PrepareAsync(cell, layers, cancellationToken);
                }
