...
namespace ZeroEngine.TCE
{
    ...
    [Serializable]
    [TceComponentDoc(TceComponentDocCategory.Condition, "zeroengine.tce.condition.chance", "Chance", "Passes based on a deterministic random source.", "Use this condition when a trigger, owner, or install source exposes ITceRandomSource. The generic package does not own project RNG state.")]
    public sealed class ChanceConditionData : TceConditionData<ChanceCondition>, ITceComponentDataValidator
    {
        [TceFieldDoc("Acceptance probability from 0 to 1.")]
        public float Chance = 1f;

        [TceFieldDoc("Object used to resolve the random source.")]
