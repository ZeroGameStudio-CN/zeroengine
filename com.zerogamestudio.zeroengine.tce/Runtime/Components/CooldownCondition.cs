...
namespace ZeroEngine.TCE
{
    ...
    [Serializable]
    [TceComponentDoc(TceComponentDocCategory.Condition, "zeroengine.tce.condition.cooldown", "Cooldown", "Prevents repeated accepted executions for a duration.", "Cooldown starts only after every condition has passed, then before effects run, so failed later conditions do not consume cooldown and synchronous reentry is blocked.")]
    public sealed class CooldownConditionData : TceConditionData<CooldownCondition>, ITceComponentDataValidator
    {
        [TceFieldDoc("Cooldown duration in domain time seconds.")]
